import { discountedPresalePriceNFT } from '@config';
...
export default task('presale', 'gets and sets presale date to now or other date')
  ...
  .addOptionalParam('price', 'update the discounted price table', false, types.boolean)
  .addOptionalParam(
    'date',
  ...
  .setAction(async ({ test, price, date }, hre) => {
